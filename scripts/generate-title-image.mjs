...
async function coverToWebp(sourcePath, targetPath, targetWidth, targetHeight) {
...
async function main() {
  ...
  const prompt = [
    spec.globalPrompt,
    titleImage.prompt,
    `Final delivery will be cropped to ${spec.fixedWidth}x${spec.fixedHeight}, so compose safely for a narrow vertical cover.`,
  ...
  await coverToWebp(rawPath, finalPath, spec.fixedWidth, spec.fixedHeight);
