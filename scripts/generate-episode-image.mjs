  ...
  for (const episode of episodes) {
    ...
    const referenceBuffers = await Promise.all(
      referenceEntries.map(async (entry) => ({
        id: entry.id,
        path: path.join(rootDir, entry.referenceImage),
        mimeType: getMimeType(path.join(rootDir, entry.referenceImage)),
        data: await readFile(path.join(rootDir, entry.referenceImage)),
      })),
    );
