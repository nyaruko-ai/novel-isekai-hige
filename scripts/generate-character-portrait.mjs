...
function buildPrompt(spec, character) {
  ...
  return [
    globalPrompt,
    character.prompt,
    "Center the subject and leave generous margin on both left and right sides for a later narrow crop.",
    ...
    `Final delivery will be cropped to ${fixedWidth}x${fixedHeight}, so compose safely.`,
...
async function generateCharacter(character, promptFile, apiKey, cliReferencePath) {
  const referencePath = cliReferencePath || defaultReferenceImages.get(character.id) || null;
